        ...
        public object this[string key]
        {
            ...
            set
            {
                if (key.IndexOfAny(Separators) == -1)
                {
                    ...
                    scope.SetMemory(_dialogContext, JsonSerializer.SerializeToElement(value, ProtocolJsonSerializer.SerializationOptions));
                }
                else
                {
                    SetValue(key, value);
...
#pragma warning restore CA1031 // Do not catch general exception types
            ...
            // Workaround to support .First() retrieval on turn.recognized.entities.foo, replace with Expressions once expression ship
        ...
        public void SetValue(string path, object value)
        {
            ...
            if (value != null)
            {
                value = JsonSerializer.SerializeToNode(value, ProtocolJsonSerializer.SerializationOptions);
            }
        ...
        public JsonObject GetMemorySnapshot()
        {
            ...
            foreach (var scope in Configuration.MemoryScopes.Where(ms => ms.IncludeInSnapshot))
            {
                ...
                if (memory != null)
                {
                    var jsonObject = JsonObject.Create(JsonSerializer.SerializeToElement(memory, ProtocolJsonSerializer.SerializationOptions));
                    result[scope.Name] = jsonObject;
                }
            }
        ...
        {
            if (ObjectPath.TryGetPathValue<JsonArray>(memory, remainingPath, out var array))
            {
                if (array != null && array.Count > 0)
                {
                    if (array[0] is JsonArray first)
                    {
                        if (first.Count > 0)
                        {
                            var second = first[0];
